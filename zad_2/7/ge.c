                         ...
                         const double *pivot_row, const double *mult)
{
  int i, j;
  for (j = 0; j + NR <= n; j += NR) {
    __m256d p0123 = _mm256_loadu_pd(pivot_row + j);
    __m256d p4567 = _mm256_loadu_pd(pivot_row + j + 4);
    for (i = 0; i < m; i++) {
      double *c = C + i * ldc + j;
      __m256d mi = _mm256_set1_pd(mult[i]);
      _mm256_storeu_pd(c,     _mm256_fnmadd_pd(p0123, mi, _mm256_loadu_pd(c)));
      _mm256_storeu_pd(c + 4, _mm256_fnmadd_pd(p4567, mi, _mm256_loadu_pd(c + 4)));
    }
  }
  for (; j < n; j++) {
